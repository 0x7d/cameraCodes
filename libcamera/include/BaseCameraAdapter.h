...
public:
	...
	virtual void enableMsgType(int32_t msgs, frame_callback callback = NULL,
			event_callback eventCb = NULL, void* cookie = NULL);
	...
	virtual status_t registerImageReleaseCallback(
			release_image_buffers_callback callback, void *user_data);

	virtual status_t registerEndCaptureCallback(
...
protected:
	...
	virtual status_t fillThisBuffer(void* frameBuf,
	...
	virtual status_t getFrameDataSize(size_t &dataFrameSize,
			size_t bufferCount);
	...
	void setFrameRefCount(void* frameBuf, CameraFrame::FrameType frameType,
			int refCount);
