...
#define LOG_TAG "BaseCameraAdapter"
...
namespace android {
	...
	status_t BaseCameraAdapter::registerImageReleaseCallback(
			release_image_buffers_callback callback, void *user_data) {
	...
	status_t BaseCameraAdapter::registerEndCaptureCallback(
			end_image_capture_callback callback, void *user_data) {
		status_t ret = NO_ERROR;
	...
	void BaseCameraAdapter::enableMsgType(int32_t msgs, frame_callback callback,
	...
	void BaseCameraAdapter::addFramePointers(void *frameBuf, void *buf) {
		...
		if ((frameBuf != NULL) && (pBuf != NULL)) {
			...
			LOGE("Adding Frame=0x%x Y=0x%x UV=0x%x", frame->mBuffer, frame->mYuv[0],
					frame->mYuv[1]);
		}
	}
	...
	void BaseCameraAdapter::removeFramePointers() {
		...
		for (int i = 0; i < size; i++) {
			...
			LOGE("Free Frame=0x%x Y=0x%x UV=0x%x", frame->mBuffer, frame->mYuv[0],
					frame->mYuv[1]);
			delete frame;
		}
		mFrameQueue.clear();
	...
	status_t BaseCameraAdapter::sendCommand(CameraCommands operation, int value1,
		...
		case CameraAdapter::CAMERA_USE_BUFFERS_PREVIEW:
			...
			if (NULL != desc) {
				ret = useBuffers(CameraAdapter::CAMERA_PREVIEW, desc->mBuffers,
						desc->mCount, desc->mLength, desc->mMaxQueueable);
			}
		...
		case CameraAdapter::CAMERA_USE_BUFFERS_PREVIEW_DATA:
			...
			if (NULL != desc) {
				ret = useBuffers(CameraAdapter::CAMERA_MEASUREMENT, desc->mBuffers,
						desc->mCount, desc->mLength, desc->mMaxQueueable);
			}
		...
		case CameraAdapter::CAMERA_USE_BUFFERS_IMAGE_CAPTURE:
			...
			if (NULL != desc) {
				ret = useBuffers(CameraAdapter::CAMERA_IMAGE_CAPTURE,
						desc->mBuffers, desc->mCount, desc->mLength,
						desc->mMaxQueueable);
			}
	...
	status_t BaseCameraAdapter::notifyZoomSubscribers(int zoomIdx,
			bool targetReached) {
		event_callback eventCb;
	...
	status_t BaseCameraAdapter::sendFrameToSubscribers(CameraFrame *frame) {
		...
		case CameraFrame::IMAGE_FRAME: {
						       ret = __sendFrameToSubscribers(frame, &mImageSubscribers,
								       CameraFrame::IMAGE_FRAME);
					       }
					       break;
		case CameraFrame::RAW_FRAME: {
						     ret = __sendFrameToSubscribers(frame, &mRawSubscribers,
								     CameraFrame::RAW_FRAME);
					     }
					     break;
		...
		case CameraFrame::VIDEO_FRAME_SYNC: {
							    ret = __sendFrameToSubscribers(frame, &mVideoSubscribers,
									    CameraFrame::VIDEO_FRAME_SYNC);
						    }
						    break;
		case CameraFrame::FRAME_DATA_SYNC: {
							   ret = __sendFrameToSubscribers(frame, &mFrameDataSubscribers,
									   CameraFrame::FRAME_DATA_SYNC);
						   }
						   break;
		default:
						   LOGE("FRAMETYPE NOT SUPPORTED");
						   break;
		}
		if (ret != NO_ERROR) {
			goto EXIT;
...
EXIT: return ret;
	...
	status_t BaseCameraAdapter::__sendFrameToSubscribers(CameraFrame* frame,
		...
		if (NULL != subscribers) {
			...
			/*
			   LOGE("Subcribers size %d", subscribers->size());
	...
	int BaseCameraAdapter::setInitFrameRefCount(void* buf, unsigned int mask) {
		...
		for (lmask = 1; lmask < CameraFrame::ALL_FRAMES; lmask <<= 1) {
			if (lmask & mask) {
				...
				case CameraFrame::IMAGE_FRAME: {
								       setFrameRefCount(buf, CameraFrame::IMAGE_FRAME,
										       (int) mImageSubscribers.size());
							       }
							       break;
				case CameraFrame::RAW_FRAME: {
								     setFrameRefCount(buf, CameraFrame::RAW_FRAME,
										     mRawSubscribers.size());
							     }
							     break;
				case CameraFrame::PREVIEW_FRAME_SYNC: {
									      setFrameRefCount(buf, CameraFrame::PREVIEW_FRAME_SYNC,
											      mFrameSubscribers.size());
								      }
								      break;
				case CameraFrame::SNAPSHOT_FRAME: {
								  ...
								  break;
				case CameraFrame::VIDEO_FRAME_SYNC: {
									    setFrameRefCount(buf, CameraFrame::VIDEO_FRAME_SYNC,
											    mVideoSubscribers.size());
								    }
								    break;
				case CameraFrame::FRAME_DATA_SYNC: {
									   setFrameRefCount(buf, CameraFrame::FRAME_DATA_SYNC,
											   mFrameDataSubscribers.size());
								   }
								   break;
				default:
	...
	int BaseCameraAdapter::getFrameRefCount(void* frameBuf,
			CameraFrame::FrameType frameType) {
		int res = -1;
	...
	void BaseCameraAdapter::setFrameRefCount(void* frameBuf,
			CameraFrame::FrameType frameType, int refCount) {
		...
		case CameraFrame::RAW_FRAME: {
						     Mutex::Autolock lock(mCaptureBufferLock);
						     mCaptureBuffersAvailable.replaceValueFor((unsigned int) frameBuf,
								     refCount);
					     }
					     break;
		case CameraFrame::PREVIEW_FRAME_SYNC:
		case CameraFrame::SNAPSHOT_FRAME: {
							  Mutex::Autolock lock(mPreviewBufferLock);
							  mPreviewBuffersAvailable.replaceValueFor((unsigned int) frameBuf,
									  refCount);
						  }
						  break;
		case CameraFrame::FRAME_DATA_SYNC: {
							   Mutex::Autolock lock(mPreviewDataBufferLock);
							   mPreviewDataBuffersAvailable.replaceValueFor((unsigned int) frameBuf,
									   refCount);
						   }
						   break;
		case CameraFrame::VIDEO_FRAME_SYNC: {
							    Mutex::Autolock lock(mVideoBufferLock);
							    mVideoBuffersAvailable.replaceValueFor((unsigned int) frameBuf,
									    refCount);
						    }
						    break;
		default:
	...
	status_t BaseCameraAdapter::useBuffers(CameraMode mode, void* bufArr, int num,
	...
	status_t BaseCameraAdapter::fillThisBuffer(void* frameBuf,
			CameraFrame::FrameType frameType) {
		status_t ret = NO_ERROR;
	...
	status_t BaseCameraAdapter::getFrameDataSize(size_t &dataFrameSize,
			size_t bufferCount) {
		status_t ret = NO_ERROR;
	...
	status_t BaseCameraAdapter::getPictureBufferSize(size_t &length,
			size_t bufferCount) {
		status_t ret = NO_ERROR;
	...
	status_t BaseCameraAdapter::setState(CameraCommands operation) {
		...
		case INTIALIZED_STATE:
			...
			case CAMERA_USE_BUFFERS_PREVIEW:
				LOGE(
						"Adapter state switch INTIALIZED_STATE->LOADED_PREVIEW_STATE event = 0x%x",
						operation);
			...
			case CAMERA_QUERY_BUFFER_SIZE_PREVIEW_DATA:
				LOGE(
						"Adapter state switch INTIALIZED_STATE->INTIALIZED_STATE event = 0x%x",
						operation);
			...
			default:
				LOGE(
						"Adapter state switch INTIALIZED_STATE Invalid Op! event = 0x%x",
						operation);
		...
		case LOADED_PREVIEW_STATE:
			...
			case CAMERA_START_PREVIEW:
				LOGE(
						"Adapter state switch LOADED_PREVIEW_STATE->PREVIEW_STATE event = 0x%x",
						operation);
			...
			case CAMERA_USE_BUFFERS_PREVIEW_DATA:
				LOGE(
						"Adapter state switch LOADED_PREVIEW_STATE->LOADED_PREVIEW_STATE event = 0x%x",
						operation);
				mNextState = LOADED_PREVIEW_STATE;
				break;
		...
		case PREVIEW_STATE:
			...
			case CAMERA_STOP_PREVIEW:
				LOGE(
						"Adapter state switch PREVIEW_STATE->INTIALIZED_STATE event = 0x%x",
						operation);
				mNextState = INTIALIZED_STATE;
				break;
			...
			case CAMERA_USE_BUFFERS_IMAGE_CAPTURE:
				LOGE(
						"Adapter state switch PREVIEW_STATE->LOADED_CAPTURE_STATE event = 0x%x",
						operation);
			...
			case CAMERA_STOP_SMOOTH_ZOOM:
				LOGE(
						"Adapter state switch PREVIEW_ACTIVE->PREVIEW_ACTIVE event = 0x%x",
						operation);
				mNextState = PREVIEW_STATE;
				break;
		...
		case LOADED_CAPTURE_STATE:
			...
			case CAMERA_START_IMAGE_CAPTURE:
				LOGE(
						"Adapter state switch LOADED_CAPTURE_STATE->CAPTURE_STATE event = 0x%x",
						operation);
			...
			case CAMERA_START_BRACKET_CAPTURE:
				LOGE(
						"Adapter state switch LOADED_CAPTURE_STATE->BRACKETING_STATE event = 0x%x",
						operation);
			...
			default:
				LOGE(
						"Adapter state switch LOADED_CAPTURE_STATE Invalid Op! event = 0x%x",
						operation);
				ret = INVALID_OPERATION;
				break;
		...
		case CAPTURE_STATE:
			...
			case CAMERA_STOP_BRACKET_CAPTURE:
				LOGE(
						"Adapter state switch CAPTURE_STATE->PREVIEW_STATE event = 0x%x",
						operation);
				mNextState = PREVIEW_STATE;
				break;
		...
		case BRACKETING_STATE:
			...
			case CAMERA_STOP_BRACKET_CAPTURE:
				LOGE(
						"Adapter state switch BRACKETING_STATE->PREVIEW_STATE event = 0x%x",
						operation);
				mNextState = PREVIEW_STATE;
				break;
			...
			case CAMERA_START_IMAGE_CAPTURE:
				LOGE(
						"Adapter state switch BRACKETING_STATE->CAPTURE_STATE event = 0x%x",
						operation);
				mNextState = CAPTURE_STATE;
				break;
			...
			default:
				LOGE(
						"Adapter state switch BRACKETING_STATE Invalid Op! event = 0x%x",
						operation);
				ret = INVALID_OPERATION;
				break;
		...
		case ZOOM_STATE:
			...
			case CAMERA_START_VIDEO:
				LOGE(
						"Adapter state switch ZOOM_STATE->VIDEO_ZOOM_STATE event = 0x%x",
						operation);
		...
		case VIDEO_STATE:
			...
			case CAMERA_PERFORM_AUTOFOCUS:
				LOGE(
						"Adapter state switch VIDEO_STATE->VIDEO_AF_STATE event = 0x%x",
						operation);
			...
			case CAMERA_START_SMOOTH_ZOOM:
				LOGE(
						"Adapter state switch VIDEO_STATE->VIDEO_ZOOM_STATE event = 0x%x",
						operation);
				mNextState = VIDEO_ZOOM_STATE;
				break;
			...
			case CAMERA_USE_BUFFERS_IMAGE_CAPTURE:
				LOGE(
						"Adapter state switch VIDEO_STATE->VIDEO_LOADED_CAPTURE_STATE event = 0x%x",
						operation);
		...
		case VIDEO_AF_STATE:
			...
			case CAMERA_CANCEL_AUTOFOCUS:
				LOGE(
						"Adapter state switch VIDEO_AF_STATE->VIDEO_STATE event = 0x%x",
						operation);
				mNextState = VIDEO_STATE;
				break;
		...
		case VIDEO_ZOOM_STATE:
			...
			case CAMERA_STOP_SMOOTH_ZOOM:
				LOGE(
						"Adapter state switch VIDEO_ZOOM_STATE->VIDEO_STATE event = 0x%x",
						operation);
				mNextState = VIDEO_STATE;
				break;
			...
			case CAMERA_STOP_VIDEO:
				LOGE(
						"Adapter state switch VIDEO_ZOOM_STATE->ZOOM_STATE event = 0x%x",
						operation);
				mNextState = ZOOM_STATE;
				break;
			...
			default:
				LOGE(
						"Adapter state switch VIDEO_ZOOM_STATE Invalid Op! event = 0x%x",
						operation);
				ret = INVALID_OPERATION;
				break;
		...
		case BRACKETING_ZOOM_STATE:
			...
			case CAMERA_STOP_SMOOTH_ZOOM:
				LOGE(
						"Adapter state switch BRACKETING_ZOOM_STATE->BRACKETING_STATE event = 0x%x",
						operation);
				mNextState = BRACKETING_STATE;
				break;
			...
			default:
				LOGE(
						"Adapter state switch BRACKETING_ZOOM_STATE Invalid Op! event = 0x%x",
						operation);
				ret = INVALID_OPERATION;
				break;
	...
	void BaseCameraAdapter::onOrientationEvent(uint32_t orientation,
			uint32_t tilt) {
		LOG_FUNCTION_NAME;
		LOG_FUNCTION_NAME_EXIT;
	}
