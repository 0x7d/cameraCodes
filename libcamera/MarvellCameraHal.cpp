...
#define LOG_TAG "MarvellCameraHal"

#include <utils/threads.h>
#include "CameraHal.h"
#include "CameraProperties.h"

static android::CameraProperties mCameraProperties;
...
static camera_info HAL_cameraInfo[1] = { { CAMERA_FACING_FRONT, 180, } };
...
void camera_set_callbacks(struct camera_device * device,
	...
	gCameraHals[ti_dev->cameraid]->setCallbacks(notify_cb, data_cb,
...
int camera_device_open(const hw_module_t* module, const char* name,
	...
	if (name != NULL) {
		cameraid = atoi(name);
		num_cameras = mCameraProperties.camerasSupported();
		...
		*device = &camera_device->base.common;
		...
		if (mCameraProperties.getProperties(cameraid, &properties) < 0) {
		...
		if (properties
				&& (camera->initialize(properties) != android::NO_ERROR)) {
...
int camera_get_camera_info(int camera_id, struct camera_info *info) {
	LOGE("camera_get_camera_info, camera_id %d\n", camera_id);

	android::CameraProperties::Properties* properties = NULL;
	if (mCameraProperties.initialize() != android::NO_ERROR) {
		LOGE("Unable to create or initialize CameraProperties");
		return NULL;
	}
	
	if (mCameraProperties.getProperties(camera_id, &properties) < 0) {
		LOGE("Couldn't get camera properties");
		return -EINVAL;
	}
