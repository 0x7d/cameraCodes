...
status_t OMXCameraAdapter::initialize(CameraProperties::Properties* caps)
{
    ...
    if ( 0 != mInitSem.Count() )
        {
        LOGE("Error mInitSem semaphore count %d", mInitSem.Count());
        LOG_FUNCTION_NAME_EXIT;
        return NO_INIT;
        }

    if (mComponentState != OMX_StateLoaded && mComponentState != OMX_StateInvalid) {
       LOGE("Error mComponentState %d is invalid!", mComponentState);
       LOG_FUNCTION_NAME_EXIT;
       return NO_INIT;
    }
    ...
    if (eError != OMX_ErrorNone) {
      LOGE("Error OMX_Init -0x%x", eError);
      return eError;
    }
    ...
    if(eError != OMX_ErrorNone) {
        LOGE("OMX_GetHandle -0x%x", eError);
    }
    GOTO_EXIT_IF((eError != OMX_ErrorNone), eError);

    LOGE("OMX_GetHandle -0x%x sensor_index = %lu", eError, mSensorIndex);
    ...
    if(eError != OMX_ErrorNone) {
         LOGE("OMX_SendCommand(OMX_CommandPortDisable) -0x%x", eError);
    }
    GOTO_EXIT_IF((eError != OMX_ErrorNone), eError);
    ...
    ret = RegisterForEvent(mCameraAdapterParameters.mHandleComp,
                                 ...
                                 mInitSem);
    if(ret != NO_ERROR) {
         LOGE("Error in registering for event %d", ret);
         goto EXIT;
    }
    ...
    eError = OMX_SendCommand(mCameraAdapterParameters.mHandleComp,
                                 ...
                                 NULL);
    if(eError != OMX_ErrorNone) {
        LOGE("OMX_SendCommand(OMX_CommandPortEnable) -0x%x", eError);
    }
    GOTO_EXIT_IF((eError!=OMX_ErrorNone), eError);
    ...
    if ( NO_ERROR == ret ) {
         LOGE("-Port enable event arrived");
    } else {
         ret |= RemoveEvent(mCameraAdapterParameters.mHandleComp,
                            ...
                            mCameraAdapterParameters.mPrevPortIndex,
                            NULL);
         LOGE("Timeout for enabling preview port expired!");
         goto EXIT;
     }
    ...
    if ( OMX_ErrorNone != eError ) {
        LOGE("Error while selecting the sensor index as %d - 0x%x", mSensorIndex, eError);
        return BAD_VALUE;
    } else {
        LOGE("Sensor %d selected successfully", mSensorIndex);
    }
    ...
    {
        LOGE("Couldn't create command handler");
        return NO_MEMORY;
    }
    ...
    if ( ret != NO_ERROR )
    {
        if( ret == INVALID_OPERATION){
            LOGE("command handler thread already runnning!!");
	    ret = NO_ERROR;
        } else
        {
            LOGE("Couldn't run command handlerthread");
            return ret;
        }
    }
    ...
    if ( NO_ERROR != ret ) {
        LOGE("Couldn't apply 3A defaults!");
        goto EXIT;
    }
...
status_t OMXCameraAdapter::fillThisBuffer(void* frameBuf, CameraFrame::FrameType frameType)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( NULL == port )
            {
            LOGE("Invalid frameType 0x%x", frameType);
            ret = -EINVAL;
            }
        }
    ...
    if ( NO_ERROR == ret )
        ...
        for ( int i = 0 ; i < port->mNumBufs ; i++)
            ...
            if ( port->mBufferHeader[i]->pBuffer == frameBuf )
                ...
                if ( eError != OMX_ErrorNone )
                    {
                    LOGE("OMX_FillThisBuffer 0x%x", eError);
                    goto EXIT;
                    }
                mFramesWithDucati++;
                break;
...
status_t OMXCameraAdapter::setParameters(const CameraParameters &params)
{
    ...
    if ( ( 0 < minFramerate ) &&
        ...
        if ( minFramerate > maxFramerate )
            {
             LOGE(" Min FPS set higher than MAX. So setting MIN and MAX to the higher value");
    ...
    if ( 0 < frameRate )
        ...
        LOGE("Prev: cap.mColorFormat = %d", (int)cap->mColorFormat);
        LOGE("Prev: cap.mWidth = %d", (int)cap->mWidth);
        LOGE("Prev: cap.mHeight = %d", (int)cap->mHeight);
        LOGE("Prev: cap.mFrameRate = %d", (int)cap->mFrameRate);
       ...
       //use preview resolution for now
        ...
        LOGE("Video: cap.mColorFormat = %d", (int)cap->mColorFormat);
        LOGE("Video: cap.mWidth = %d", (int)cap->mWidth);
        LOGE("Video: cap.mHeight = %d", (int)cap->mHeight);
        LOGE("Video: cap.mFrameRate = %d", (int)cap->mFrameRate);
...
void saveFile(unsigned char   *buff, int width, int height, int format) {
    ...
    LOGE("Copying from 0x%x, size=%d x %d", buff, width, height);
...
void OMXCameraAdapter::getParameters(CameraParameters& params)
{
    ...
    if ( OMX_ErrorNone == eError )
        ...
        {
        LOGE("OMX error 0x%x, while retrieving current ISO value", eError);
        }
    ...
    if ( ZOOM_ACTIVE & state )
        ...
        LOGE("CameraParameters Zoom = %d", mCurrentZoomIdx);
        }
    else
        {
    ...
    if( (valstr = mParams.get(CameraParameters::KEY_AUTO_EXPOSURE_LOCK)) != NULL )
      {
        LOGE("Auto Exposure Lock get %s", mParams.get(CameraParameters::KEY_AUTO_EXPOSURE_LOCK));
    ...
    if( (valstr = mParams.get(CameraParameters::KEY_AUTO_WHITEBALANCE_LOCK)) != NULL )
      {
        LOGE("Auto WhiteBalance Lock get %s", mParams.get(CameraParameters::KEY_AUTO_WHITEBALANCE_LOCK));
...
status_t OMXCameraAdapter::setFormat(OMX_U32 port, OMXCameraPortParameters &portParams)
{
    ...
    if(eError!=OMX_ErrorNone)
        {
        LOGE("OMX_GetParameter - %x", eError);
        }
    GOTO_EXIT_IF((eError!=OMX_ErrorNone), eError);
    ...
    if ( OMX_CAMERA_PORT_VIDEO_OUT_PREVIEW == port )
        ...
        {
        LOGE("Unsupported port index 0x%x", (unsigned int)port);
        }
    ...
    if(eError!=OMX_ErrorNone)
        {
        LOGE("OMX_SetParameter - %x", eError);
        }
    GOTO_EXIT_IF((eError!=OMX_ErrorNone), eError);
    ...
    LOGE("Exiting function %s because of eError=%x", __FUNCTION__, eError);
...
status_t OMXCameraAdapter::flushBuffers()
{
    ...
    if ( 0 != mFlushSem.Count() )
        {
        LOGE("Error mFlushSem semaphore count %d", mFlushSem.Count());
        LOG_FUNCTION_NAME_EXIT;
        return NO_INIT;
        }
    ...
    if(eError!=OMX_ErrorNone)
        {
        LOGE("OMX_SendCommand(OMX_CommandFlush)-0x%x", eError);
        }
    GOTO_EXIT_IF((eError!=OMX_ErrorNone), eError);

    LOGE("Waiting for flush event");
    ...
    if (mComponentState == OMX_StateInvalid)
      {
        LOGE("Invalid State after Flush Exitting!!!");
        goto EXIT;
      }

    if ( NO_ERROR == ret )
        {
        LOGE("Flush event received");
        }
    else
        {
        ret |= RemoveEvent(mCameraAdapterParameters.mHandleComp,
                           ...
                           OMX_CAMERA_PORT_VIDEO_OUT_PREVIEW,
                           NULL);
        LOGE("Flush event timeout expired");
        goto EXIT;
        }
...
status_t OMXCameraAdapter::UseBuffersPreviewData(void* bufArr, int num)
{
    ...
    if ( mComponentState != OMX_StateLoaded )
        {
        LOGE("Calling UseBuffersPreviewData() when not in LOADED state");
        return BAD_VALUE;
        }

    if ( NULL == bufArr )
        {
        LOGE("NULL pointer passed for buffArr");
        return BAD_VALUE;
        }

    if ( 0 != mUsePreviewDataSem.Count() )
        {
        LOGE("Error mUsePreviewDataSem semaphore count %d", mUsePreviewDataSem.Count());
        LOG_FUNCTION_NAME_EXIT;
        return NO_INIT;
        }
    ...
    if ( NO_ERROR == ret )
        ...
        if ( ret == NO_ERROR )
            {
            LOGE("Registering for event %d", ret);
            }
        else
            {
            LOGE("Error in registering for event %d", ret);
    ...
    if ( NO_ERROR == ret )
        ...
        if (mComponentState == OMX_StateInvalid)
          {
            LOGE("Invalid State after measurement port enable Exitting!!!");
            goto EXIT;
          }

        if ( NO_ERROR == ret )
            {
            LOGE("Port enable event arrived on measurement port");
            }
        else
            {
            ret |= RemoveEvent(mCameraAdapterParameters.mHandleComp,
                               ...
                               mCameraAdapterParameters.mMeasurementPortIndex,
                               NULL);
            LOGE("Timeout expoired during port enable on measurement port");
            goto EXIT;
            }
...
status_t OMXCameraAdapter::doSwitchToExecuting()
{
  ...
  if ( (mComponentState == OMX_StateExecuting) || (mComponentState == OMX_StateInvalid) ){
    LOGE("Already in OMX_Executing state or OMX_StateInvalid state");
    mStateSwitchLock.unlock();
    return NO_ERROR;
  }

  if ( 0 != mSwitchToExecSem.Count() ){
    LOGE("Error mSwitchToExecSem semaphore count %d", mSwitchToExecSem.Count());
    goto EXIT;
  }
  ...
  ret = RegisterForEvent(mCameraAdapterParameters.mHandleComp,
                         ...
                         mSwitchToExecSem);
  if ( NO_ERROR != ret ){
    LOGE("Error in registering Port Disable for event %d", ret);
    goto EXIT;
  }
  ...
  if (ret != NO_ERROR){
    LOGE("Timeout PREVIEW PORT DISABLE %d", ret);
  }

  LOGE("PREV PORT DISABLED %d", ret);
  ...
  if(ret!=NO_ERROR)
    {
      LOGE("Error in IDLE STATE SWITCH %d", ret);
      goto EXIT;
    }
  eError = OMX_SendCommand (mCameraAdapterParameters.mHandleComp ,
                            OMX_CommandStateSet,
  ...
  ret = mSwitchToExecSem.WaitTimeout(OMX_CMD_TIMEOUT);
  if (ret != NO_ERROR){
    LOGE("Timeout IDLE STATE SWITCH %d", ret);
    goto EXIT;
  }
  mComponentState = OMX_StateIdle;
  LOGE("OMX_SendCommand(OMX_StateIdle) 0x%x", eError);
  ...
  if(ret!=NO_ERROR)
    {
      LOGE("Error in EXECUTING STATE SWITCH %d", ret);
      goto EXIT;
    }
  eError = OMX_SendCommand (mCameraAdapterParameters.mHandleComp ,
                            OMX_CommandStateSet,
  ...
  ret = mSwitchToExecSem.WaitTimeout(OMX_CMD_TIMEOUT);
  if (ret != NO_ERROR){
    LOGE("Timeout EXEC STATE SWITCH %d", ret);
    goto EXIT;
  }
  mComponentState = OMX_StateExecuting;
  LOGE("OMX_SendCommand(OMX_StateExecuting) 0x%x", eError);
...
status_t OMXCameraAdapter::switchToLoaded()
{
    ...
    if ( mComponentState == OMX_StateLoaded  || mComponentState == OMX_StateInvalid)
        {
        LOGE("Already in OMX_Loaded state or OMX_StateInvalid state");
        return NO_ERROR;
        }

    if ( 0 != mSwitchToLoadedSem.Count() )
        {
        LOGE("Error mSwitchToLoadedSem semaphore count %d", mSwitchToLoadedSem.Count());
        goto EXIT;
        }
    ...
    if(eError!=OMX_ErrorNone)
        {
        LOGE("OMX_SendCommand(OMX_StateIdle) - %x", eError);
        }
    ...
    LOGE("EXECUTING->IDLE state changed");
    ret = mSwitchToLoadedSem.WaitTimeout(OMX_CMD_TIMEOUT);

    //If somethiing bad happened while we wait
    if (mComponentState == OMX_StateInvalid)
      {
        LOGE("Invalid State after EXECUTING->IDLE Exitting!!!");
        goto EXIT;
      }
    ...
    if ( NO_ERROR == ret )
        ...
        ret |= RemoveEvent(mCameraAdapterParameters.mHandleComp,
                           ...
                           OMX_StateIdle,
                           NULL);
        LOGE("Timeout expired on EXECUTING->IDLE state change");
        goto EXIT;
        }
    ...
    if(eError!=OMX_ErrorNone)
        {
        LOGE("OMX_SendCommand(OMX_StateLoaded) - %x", eError);
        }
    GOTO_EXIT_IF((eError!=OMX_ErrorNone), eError);

    LOGE("Switching IDLE->LOADED state");
    ret = mSwitchToLoadedSem.WaitTimeout(OMX_CMD_TIMEOUT);

    //If somethiing bad happened while we wait
    if (mComponentState == OMX_StateInvalid)
      {
        LOGE("Invalid State after IDLE->LOADED Exitting!!!");
        goto EXIT;
      }

    if ( NO_ERROR == ret )
        {
        LOGE("IDLE->LOADED state changed");
        }
    else
        {
        ret |= RemoveEvent(mCameraAdapterParameters.mHandleComp,
                           ...
                           OMX_StateLoaded,
                           NULL);
        LOGE("Timeout expired on IDLE->LOADED state change");
        goto EXIT;
        }
    ...
    LOGE("OMX_SendCommand(OMX_CommandStateSet) 0x%x", eError);
    GOTO_EXIT_IF((eError!=OMX_ErrorNone), eError);

    LOGE("Enabling Preview port");
    ...
    if (mComponentState == OMX_StateInvalid)
      {
        LOGE("Invalid State after Enabling Preview port Exitting!!!");
        goto EXIT;
      }

    if ( NO_ERROR == ret )
        {
        LOGE("Preview port enabled!");
        }
    else
        {
        ret |= RemoveEvent(mCameraAdapterParameters.mHandleComp,
...
status_t OMXCameraAdapter::UseBuffersPreview(void* bufArr, int num)
{
    ...
    if ( 0 != mUsePreviewSem.Count() )
        {
        LOGE("Error mUsePreviewSem semaphore count %d", mUsePreviewSem.Count());
        LOG_FUNCTION_NAME_EXIT;
        return NO_INIT;
        }
    ...
    if(mPreviewData->mNumBufs != num)
        {
        LOGE("Current number of buffers doesnt equal new num of buffers passed!");
        LOG_FUNCTION_NAME_EXIT;
        return BAD_VALUE;
        }
    ...
    if ( mComponentState == OMX_StateLoaded )
        ...
        if ( NO_ERROR != ret )
            {
            LOGE("setCaptureMode() failed %d", ret);
            LOG_FUNCTION_NAME_EXIT;
            return ret;
            }

        LOGE("Camera Mode = %d", mCapMode);
        ...
        if( ( mCapMode == OMXCameraAdapter::VIDEO_MODE ) ||
            ...
            if ( NO_ERROR != ret)
                {
                LOGE("Error configuring VNF %x", ret);
                return ret;
                }
            ...
            if ( NO_ERROR != ret)
                {
                LOGE("Error configuring VSTAB %x", ret);
                return ret;
                }
            }
        else
    ...
    if ( NO_ERROR != ret )
        {
        LOGE("Error configuring Sensor Orientation %x", ret);
    ...
    if ( ret != NO_ERROR )
        {
        LOGE("VFR configuration failed 0x%x", ret);
        LOG_FUNCTION_NAME_EXIT;
        return ret;
        }
    ...
    for(int index=0;index<num;index++) {
        ...
        LOGE("OMX_UseBuffer(0x%x)", buffers[index]);
    ...
    LOGE("Registering preview buffers");
    ...
    if (mComponentState == OMX_StateInvalid)
      {
        LOGE("Invalid State after Registering preview buffers Exitting!!!");
        goto EXIT;
      }

    if ( NO_ERROR == ret )
        {
        LOGE("Preview buffer registration successfull");
        }
    else
        {
        if ( mComponentState == OMX_StateLoaded )
        ...
        LOGE("Timeout expired on preview buffer registration");
        goto EXIT;
        }
...
status_t OMXCameraAdapter::startPreview()
{
    ...
    if( 0 != mStartPreviewSem.Count() )
        {
        LOGE("Error mStartPreviewSem semaphore count %d", mStartPreviewSem.Count());
        LOG_FUNCTION_NAME_EXIT;
        return NO_INIT;
        }
    ...
    if( OMX_StateIdle == mComponentState )
        ...
        LOGE("+Waiting for component to go into EXECUTING state");
        ret = mStartPreviewSem.WaitTimeout(OMX_CMD_TIMEOUT);

        //If somethiing bad happened while we wait
        if (mComponentState == OMX_StateInvalid)
          {
            LOGE("Invalid State after IDLE_EXECUTING Exitting!!!");
            goto EXIT;
          }

        if ( NO_ERROR == ret )
            {
            LOGE("+Great. Component went into executing state!!");
            }
        else
            {
            ret |= RemoveEvent(mCameraAdapterParameters.mHandleComp,
                               ...
                               OMX_StateExecuting,
                               NULL);
            LOGE("Timeout expired on executing state switch!");
            goto EXIT;
            }
    ...
    for(int index=0;index< mPreviewData->mMaxQueueable;index++)
        {
        LOGE("Queuing buffer on Preview port - 0x%x", (uint32_t)mPreviewData->mBufferHeader[index]->pBuffer);
    ...
    if ( mMeasurementEnabled )
        ...
        for(int index=0;index< mPreviewData->mNumBufs;index++)
            {
            LOGE("Queuing buffer on Measurement port - 0x%x", (uint32_t) measurementData->mBufferHeader[index]->pBuffer);
            eError = OMX_FillThisBuffer(mCameraAdapterParameters.mHandleComp,
...
status_t OMXCameraAdapter::stopPreview()
{
    ...
    if ( mComponentState != OMX_StateExecuting )
        {
        LOGE("Calling StopPreview() when not in EXECUTING state");
        LOG_FUNCTION_NAME_EXIT;
        return NO_INIT;
        }
    ...
    if(ret!=NO_ERROR)
    {
        LOGE("Error canceling autofocus %d", ret);
    ...
    LOGE("Configuring AF Assist mode 0x%x", focusAssist.bFocusAssist);
    eError =  OMX_SetConfig(mCameraAdapterParameters.mHandleComp,
                            (OMX_INDEXTYPE) OMX_IndexConfigFocusAssist,
                            &focusAssist);
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring AF Assist mode 0x%x", eError);
        }
    else
        {
        LOGE("Camera AF Assist  mode configured successfully");
        }

    if ( 0 != mStopPreviewSem.Count() )
        {
        LOGE("Error mStopPreviewSem semaphore count %d", mStopPreviewSem.Count());
        LOG_FUNCTION_NAME_EXIT;
        return NO_INIT;
        }
    ...
    if ( NO_ERROR != ret ) {
        LOGE("disable image port failed 0x%x", ret);
        goto EXIT;
    }

    LOGE("Average framerate: %f", mFPS);

    //Avoid state switching of the OMX Component
    ret = flushBuffers();
    if ( NO_ERROR != ret )
        {
        LOGE("Flush Buffers failed 0x%x", ret);
        goto EXIT;
        }
    ...
    LOGE("Disabling preview port");
    ret = mStopPreviewSem.WaitTimeout(OMX_CMD_TIMEOUT);

    //If somethiing bad happened while we wait
    if (mComponentState == OMX_StateInvalid)
      {
        LOGE("Invalid State after Disabling preview port Exitting!!!");
        goto EXIT;
      }

    if ( NO_ERROR == ret )
        {
        LOGE("Preview port disabled");
        }
    else
        {
        ret |= RemoveEvent(mCameraAdapterParameters.mHandleComp,
                           ...
                           mCameraAdapterParameters.mPrevPortIndex,
                           NULL);
        LOGE("Timeout expired on preview port disable");
        goto EXIT;
        }
...
status_t OMXCameraAdapter::setSensorOverclock(bool enable)
{
    ...
    if ( OMX_StateLoaded != mComponentState )
        {
        LOGE("OMX component is not in loaded state");
        return ret;
        }
...
status_t OMXCameraAdapter::printComponentVersion(OMX_HANDLETYPE handle)
{
    ...
    if ( NULL == handle )
        {
        LOGE("Invalid OMX Handle =0x%x",  ( unsigned int ) handle);
        ret = -EINVAL;
        }
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("OMX_GetComponentVersion returned 0x%x", eError);
            ret = BAD_VALUE;
            }
        }
    ...
    if ( NO_ERROR == ret )
        {
        LOGE("OMX Component name: [%s]", compName);
        LOGE("OMX Component version: [%u]", ( unsigned int ) compVersion.nVersion);
        LOGE("Spec version: [%u]", ( unsigned int ) mCompRevision.nVersion);
        LOGE("Git Commit ID: [%s]", mCompUUID);
    ...
    if ( NO_ERROR == ret )
        ...
        if ( (int)currentUUID + (int)offset - (int)mCompUUID < OMX_MAX_STRINGNAME_SIZE )
            {
            currentUUID += offset;
            LOGE("Build date and time: [%s]", currentUUID);
            }
        else
            {
            ret = BAD_VALUE;
    ...
    if ( NO_ERROR == ret )
        ...
        if ( (int)currentUUID + (int)offset - (int)mCompUUID < OMX_MAX_STRINGNAME_SIZE )
            {
            currentUUID += offset;
            LOGE("Build description: [%s]", currentUUID);
            }
        else
            {
            ret = BAD_VALUE;
...
status_t OMXCameraAdapter::getFrameSize(size_t &width, size_t &height)
{
    ...
    if ( mOMXStateSwitch )
        ...
        if ( NO_ERROR != ret )
            {
            LOGE("switchToLoaded() failed 0x%x", ret);
            goto exit;
            }
...
exit:
    ...
    LOGE("Required frame size %dx%d", width, height);
    LOG_FUNCTION_NAME_EXIT;
...
status_t OMXCameraAdapter::getFrameDataSize(size_t &dataFrameSize, size_t bufferCount)
{
    ...
    if ( OMX_StateLoaded != mComponentState )
        {
        LOGE("Calling getFrameDataSize() when not in LOADED state");
        dataFrameSize = 0;
        ret = BAD_VALUE;
        }
    ...
    if ( NO_ERROR == ret  )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("OMX_GetParameter on OMX_IndexParamPortDefinition returned: 0x%x", eError);
            dataFrameSize = 0;
            ret = BAD_VALUE;
            }
        }
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("OMX_SetParameter on OMX_IndexParamPortDefinition returned: 0x%x", eError);
            dataFrameSize = 0;
            ret = BAD_VALUE;
            }
        }
...
void OMXCameraAdapter::onOrientationEvent(uint32_t orientation, uint32_t tilt)
{
    ...
    LOGE("orientation = %d tilt = %d device_orientation = %d", orientation, tilt, mDeviceOrientation);
                                          ...
                                          OMX_IN OMX_PTR pEventData)
{
    ...
    OMX_ERRORTYPE eError = OMX_ErrorNone;
    LOGE("+OMX_Event %x, %d %d", eEvent, (int)nData1, (int)nData2);

    switch (eEvent) {
        case OMX_EventCmdComplete:
            LOGE("+OMX_EventCmdComplete %d %d", (int)nData1, (int)nData2);

            if (OMX_CommandStateSet == nData1) {
                mCameraAdapterParameters.mState = (OMX_STATETYPE) nData2;

            } else if (OMX_CommandFlush == nData1) {
                LOGE("OMX_CommandFlush received for port %d", (int)nData2);

            } else if (OMX_CommandPortDisable == nData1) {
                LOGE("OMX_CommandPortDisable received for port %d", (int)nData2);

            } else if (OMX_CommandPortEnable == nData1) {
                LOGE("OMX_CommandPortEnable received for port %d", (int)nData2);

            } else if (OMX_CommandMarkBuffer == nData1) {
                ///This is not used currently
            }

            LOGE("-OMX_EventCmdComplete");
        break;

        case OMX_EventIndexSettingChanged:
            LOGE("OMX_EventIndexSettingChanged event received data1 0x%x, data2 0x%x",
                            ( unsigned int ) nData1, ( unsigned int ) nData2);
            break;

        case OMX_EventError:
            LOGE("OMX interface failed to execute OMX command %d", (int)nData1);
            LOGE("See OMX_INDEXTYPE for reference");
            if ( NULL != mErrorNotifier && ( ( OMX_U32 ) OMX_ErrorHardware == nData1 ) && mComponentState != OMX_StateInvalid)
              {
                LOGE("***Got Fatal Error Notification***\n");
                mComponentState = OMX_StateInvalid;
                /*
        ...
        for ( unsigned int i = 0 ; i < mEventSignalQ.size() ; i++ )
            ...
            if ( NULL != msg )
                ...
                if( ( msg->command != 0 || msg->command == ( unsigned int ) ( eEvent ) )
                    ...
                    {
                    Semaphore *sem  = (Semaphore*) msg->arg3;
                    LOGE("Event matched, signalling sem");
                    mEventSignalQ.removeAt(i);
                                          ...
                                          OMX_IN Semaphore &semaphore)
{
    ...
    if ( NULL != msg )
        ...
        if ( NO_MEMORY == res )
            {
            LOGE("No ressources for inserting OMX events");
            free(msg);
            ret = -ENOMEM;
            }
        }
                                   ...
                                   OMX_IN OMX_BUFFERHEADERTYPE* pBuffHeader)
{
    ...
    if (pBuffHeader->nOutputPortIndex == OMX_CAMERA_PORT_VIDEO_OUT_PREVIEW)
        ...
        //LOGE("FBD pBuffer = 0x%x", pBuffHeader->pBuffer);
        ...
        if(mDebugFcs)
            LOGE("C[%d] D[%d] E[%d]", mFramesWithDucati, mFramesWithDisplay, mFramesWithEncoder);
        ...
        LOGE("Captured Frames: %d", mCapturedFrames);
        ...
        {
        LOGE("Frame received for non-(preview/capture/measure) port. This is yet to be supported");
        goto EXIT;
        }

    if ( NO_ERROR != stat )
        {
        LOGE("sendFrameToSubscribers error: %d", stat);
    ...
    LOGE("Exiting function %s because of ret %d eError=%x", __FUNCTION__, stat, eError);
...
status_t OMXCameraAdapter::sendCallBacks(CameraFrame frame, OMX_IN OMX_BUFFERHEADERTYPE *pBuffHeader, unsigned int mask, OMXCameraPortParameters *port)
{
  ...
  if ( NULL == port)
    {
      LOGE("Invalid portParam");
      return -EINVAL;
    }

  if ( NULL == pBuffHeader )
    {
      LOGE("Invalid Buffer header");
      return -EINVAL;
    }
  ...
  if (ret != NO_ERROR) {
     LOGE("Error in setInitFrameRefCount %d", ret);
  } else {
      ret = sendFrameToSubscribers(&frame);
  }

  LOGE("B 0x%x T %llu", frame.mBuffer, pBuffHeader->nTimeStamp);
...
bool OMXCameraAdapter::OMXCallbackHandler::Handler()
{
    ...
    while(forever){
        ...
        switch ( msg.command ) {
            ...
            case CommandHandler::COMMAND_EXIT:
            {
                LOGE("Exiting OMX callback handler");
                forever = 0;
                break;
            }
        }
...
extern "C" int CameraAdapter_Capabilities(CameraProperties::Properties* properties_array,
    ...
    if (!properties_array) {
        LOGE("invalid param: properties = 0x%p", properties_array);
        LOG_FUNCTION_NAME_EXIT;
        return -EINVAL;
    }
