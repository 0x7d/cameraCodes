...
status_t OMXCameraAdapter::setupEXIF()
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while retrieving EXIF configuration structure 0x%x", eError);
            ret = -1;
            }
        }
    ...
    if ( NO_ERROR == ret )
        ...
        if ( ( OMX_TI_TagReadWrite == exifTags->eStatusFocalLength ))
        {
            ...
            if (numerator || denominator) {
                ...
                LOGE("exifTags->ulFocalLength = [%u] [%u]",
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while setting EXIF configuration 0x%x", eError);
            ret = -1;
            }
        }
