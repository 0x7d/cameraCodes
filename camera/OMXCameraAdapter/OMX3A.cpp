                                           ...
                                           BaseCameraAdapter::AdapterState state)
{
    ...
    if ( mFirstTimeInit || ((str != NULL) && ( mParameters3A.SceneMode != mode )) ) {
        ...
        LOGE("SceneMode %d", mParameters3A.SceneMode);
    }
    ...
    if ( 0 <= params.getInt(TICameraParameters::KEY_CONTRAST) )
        ...
        if ( mFirstTimeInit ||
            ...
            LOGE("Contrast %d", mParameters3A.Contrast);
    ...
    if ( 0 <= params.getInt(TICameraParameters::KEY_SHARPNESS) )
        ...
        if ( mFirstTimeInit ||
            ...
            LOGE("Sharpness %d", mParameters3A.Sharpness);
    ...
    if ( 0 <= params.getInt(TICameraParameters::KEY_SATURATION) )
        ...
        if ( mFirstTimeInit ||
            ...
            LOGE("Saturation %d", mParameters3A.Saturation);
    ...
    if ( 0 <= params.getInt(TICameraParameters::KEY_BRIGHTNESS) )
        ...
        if ( mFirstTimeInit ||
            ...
            LOGE("Brightness %d", mParameters3A.Brightness);
    ...
    if ( mFirstTimeInit || ( ( str != NULL ) && ( mParameters3A.Flicker != mode ) ))
        ...
        LOGE("Flicker %d", mParameters3A.Flicker);
    ...
    if ( (mFirstTimeInit || ((str != NULL) && (mParameters3A.Focus != mode))))
        ...
        LOGE("Focus %x", mParameters3A.Focus);
        }
    ...
    if ( mFirstTimeInit ||
        ...
        {
        LOGE("Setting EV Compensation to %d",
                     params.getInt(CameraParameters::KEY_EXPOSURE_COMPENSATION));
    ...
    if (  mFirstTimeInit || (( str != NULL ) && ( mParameters3A.Effect != mode )) )
        ...
        LOGE("Effect %d", mParameters3A.Effect);
    ...
    if ( (str != NULL) ) {
        ...
        if ( (NO_ERROR == ret) && CameraArea::areAreasDifferent(mMeteringAreas, tempAreas) ) {
            ...
            if ( MAX_METERING_AREAS >= mMeteringAreas.size() ) {
                LOGE("Setting Metering Areas %s",
                        params.get(CameraParameters::KEY_METERING_AREAS));

                mPending3Asettings |= SetMeteringAreas;
            } else {
                LOGE("Metering areas supported %d, metering areas set %d",
...
status_t OMXCameraAdapter::setExposureMode(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring exposure mode 0x%x", eError);
        }
    else
        {
        LOGE("Camera exposure mode configured successfully");
        }
...
status_t OMXCameraAdapter::setFlashMode(Gen3A_settings& Gen3A)
{
    ...
    LOGE("Configuring flash mode 0x%x", flash.eFlashControl);
    eError =  OMX_SetConfig(mCameraAdapterParameters.mHandleComp,
                            (OMX_INDEXTYPE) OMX_IndexConfigFlashControl,
                            &flash);
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring flash mode 0x%x", eError);
        }
    else
        {
        LOGE("Camera flash mode configured successfully");
        }
...
status_t OMXCameraAdapter::getFlashMode(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError ) {
        LOGE("Error while getting flash mode 0x%x", eError);
    } else {
        Gen3A.FlashMode = flash.eFlashControl;
        LOGE("Gen3A.FlashMode 0x%x", Gen3A.FlashMode);
    }
...
status_t OMXCameraAdapter::setFocusMode(Gen3A_settings& Gen3A)
{
    ...
    if ( NO_ERROR == ret && ((state & AF_ACTIVE) == 0) )
        ...
        LOGE("Configuring focus mode 0x%x", focus.eFocusControl);
        eError = OMX_SetConfig( mCameraAdapterParameters.mHandleComp, OMX_IndexConfigFocusControl, &focus);
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while configuring focus mode 0x%x", eError);
            }
        else
            {
            LOGE("Camera focus mode configured successfully");
            }
        }
...
status_t OMXCameraAdapter::getFocusMode(Gen3A_settings& Gen3A)
{
    ...
    if (OMX_ErrorNone != eError) {
        ...
        LOGE("Gen3A.Focus 0x%x", Gen3A.Focus);
    }
...
status_t OMXCameraAdapter::setScene(Gen3A_settings& Gen3A)
{
    ...
    LOGE("Configuring scene mode 0x%x", scene.eSceneMode);
    eError =  OMX_SetConfig(mCameraAdapterParameters.mHandleComp,
                            ( OMX_INDEXTYPE ) OMX_TI_IndexConfigSceneMode,
                            &scene);

    if (OMX_ErrorNone != eError) {
        LOGE("Error while configuring scene mode 0x%x", eError);
    } else {
        LOGE("Camera scene configured successfully");
...
status_t OMXCameraAdapter::setEVCompensation(Gen3A_settings& Gen3A)
{
    ...
    OMX_GetConfig( mCameraAdapterParameters.mHandleComp,
                   ...
                   &expValues);
    LOGE("old EV Compensation for OMX = 0x%x", (int)expValues.xEVCompensation);
    LOGE("EV Compensation for HAL = %d", Gen3A.EVCompensation);

    expValues.xEVCompensation = ( Gen3A.EVCompensation * ( 1 << Q16_OFFSET ) )  / 10;
    eError = OMX_SetConfig( mCameraAdapterParameters.mHandleComp,
                            OMX_IndexConfigCommonExposureValue,
                            &expValues);
    LOGE("new EV Compensation for OMX = 0x%x", (int)expValues.xEVCompensation);
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring EV Compensation 0x%x error = 0x%x",
                     ( unsigned int ) expValues.xEVCompensation,
                     eError);
        }
    else
        {
        LOGE("EV Compensation 0x%x configured successfully",
                     ( unsigned int ) expValues.xEVCompensation);
        }
...
status_t OMXCameraAdapter::getEVCompensation(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError ) {
        LOGE("Error while getting EV Compensation error = 0x%x", eError);
    } else {
        Gen3A.EVCompensation = (10 * expValues.xEVCompensation) / (1 << Q16_OFFSET);
        LOGE("Gen3A.EVCompensation 0x%x", Gen3A.EVCompensation);
    }
...
status_t OMXCameraAdapter::setWBMode(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring Whitebalance mode 0x%x error = 0x%x",
                     ( unsigned int ) wb.eWhiteBalControl,
                     eError);
        }
    else
        {
        LOGE("Whitebalance mode 0x%x configured successfully",
                     ( unsigned int ) wb.eWhiteBalControl);
        }
...
status_t OMXCameraAdapter::getWBMode(Gen3A_settings& Gen3A)
{
    ...
    if (OMX_ErrorNone != eError) {
        LOGE("Error while getting Whitebalance mode error = 0x%x", eError);
    } else {
        Gen3A.WhiteBallance = wb.eWhiteBalControl;
        LOGE("Gen3A.WhiteBallance 0x%x", Gen3A.WhiteBallance);
    }
...
status_t OMXCameraAdapter::setFlicker(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring Flicker mode 0x%x error = 0x%x",
                     ( unsigned int ) flicker.eFlickerCancel,
                     eError);
        }
    else
        {
        LOGE("Flicker mode 0x%x configured successfully",
                     ( unsigned int ) flicker.eFlickerCancel);
        }
...
status_t OMXCameraAdapter::setBrightness(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring Brightness 0x%x error = 0x%x",
                     ( unsigned int ) brightness.nBrightness,
                     eError);
        }
    else
        {
        LOGE("Brightness 0x%x configured successfully",
                     ( unsigned int ) brightness.nBrightness);
        }
...
status_t OMXCameraAdapter::setContrast(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring Contrast 0x%x error = 0x%x",
                     ( unsigned int ) contrast.nContrast,
                     eError);
        }
    else
        {
        LOGE("Contrast 0x%x configured successfully",
                     ( unsigned int ) contrast.nContrast);
        }
...
status_t OMXCameraAdapter::setSharpness(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring Sharpness 0x%x error = 0x%x",
                     ( unsigned int ) procSharpness.nLevel,
                     eError);
        }
    else
        {
        LOGE("Sharpness 0x%x configured successfully",
                     ( unsigned int ) procSharpness.nLevel);
        }
...
status_t OMXCameraAdapter::getSharpness(Gen3A_settings& Gen3A)
{
    ...
    if (OMX_ErrorNone != eError) {
        LOGE("Error while configuring Sharpness error = 0x%x", eError);
    } else {
        Gen3A.Sharpness = procSharpness.nLevel;
        LOGE("Gen3A.Sharpness 0x%x", Gen3A.Sharpness);
    }
...
status_t OMXCameraAdapter::setSaturation(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring Saturation 0x%x error = 0x%x",
                     ( unsigned int ) saturation.nSaturation,
                     eError);
        }
    else
        {
        LOGE("Saturation 0x%x configured successfully",
                     ( unsigned int ) saturation.nSaturation);
        }
...
status_t OMXCameraAdapter::getSaturation(Gen3A_settings& Gen3A)
{
    ...
    if (OMX_ErrorNone != eError) {
        LOGE("Error while getting Saturation error = 0x%x", eError);
    } else {
        Gen3A.Saturation = saturation.nSaturation;
        LOGE("Gen3A.Saturation 0x%x", Gen3A.Saturation);
    }
...
status_t OMXCameraAdapter::setISO(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring ISO 0x%x error = 0x%x",
                     ( unsigned int ) expValues.nSensitivity,
                     eError);
        }
    else
        {
        LOGE("ISO 0x%x configured successfully",
                     ( unsigned int ) expValues.nSensitivity);
        }
...
status_t OMXCameraAdapter::getISO(Gen3A_settings& Gen3A)
{
    ...
    if (OMX_ErrorNone != eError) {
        LOGE("Error while getting ISO error = 0x%x", eError);
    } else {
        Gen3A.ISO = expValues.nSensitivity;
        LOGE("Gen3A.ISO %d", Gen3A.ISO);
    }
...
status_t OMXCameraAdapter::setEffect(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError )
        {
        LOGE("Error while configuring Effect 0x%x error = 0x%x",
                     ( unsigned int )  effect.eImageFilter,
                     eError);
        }
    else
        {
        LOGE("Effect 0x%x configured successfully",
                     ( unsigned int )  effect.eImageFilter);
        }
...
status_t OMXCameraAdapter::setWhiteBalanceLock(Gen3A_settings& Gen3A)
{
  ...
  if ( OMX_ErrorNone != eError )
    {
      LOGE("Error while configuring WhiteBalance Lock error = 0x%x", eError);
    }
  else
    {
      LOGE("WhiteBalance Lock configured successfully %d ", lock.bLock);
    }
  LOG_FUNCTION_NAME_EXIT
...
status_t OMXCameraAdapter::setExposureLock(Gen3A_settings& Gen3A)
{
  ...
  if ( OMX_ErrorNone != eError )
    {
      LOGE("Error while configuring Exposure Lock error = 0x%x", eError);
    }
  else
    {
      LOGE("Exposure Lock configured successfully %d ", lock.bLock);
    }
  LOG_FUNCTION_NAME_EXIT
...
status_t OMXCameraAdapter::setFocusLock(Gen3A_settings& Gen3A)
{
    ...
    if ( OMX_ErrorNone != eError ) {
        LOGE("Error while configuring Focus Lock error = 0x%x", eError);
    } else {
        LOGE("Focus Lock configured successfully %d ", lock.bLock);
    }
...
status_t OMXCameraAdapter::set3ALock(OMX_BOOL toggleExp, OMX_BOOL toggleWb, OMX_BOOL toggleFocus)
{
    ...
    if ( OMX_ErrorNone != eError )
    {
        LOGE("Error GetConfig Exposure Lock error = 0x%x", eError);
        goto EXIT;
    }
    else
    {
        const char *lock_state_exp = toggleExp ? TRUE : FALSE;
        LOGE("Exposure Lock GetConfig successfull");
    ...
    if ( OMX_ErrorNone != eError )
    {
        LOGE("Error GetConfig Focus Lock error = 0x%x", eError);
        goto EXIT;
    }
    else
    {
        LOGE("Focus Lock GetConfig successfull bLock(%d)", lock.bLock);
    ...
    if ( OMX_ErrorNone != eError )
    {
        LOGE("Error GetConfig WhiteBalance Lock error = 0x%x", eError);
        goto EXIT;
    }
    else
    {
        const char *lock_state_wb = toggleWb ? TRUE : FALSE;
        LOGE("WhiteBalance Lock GetConfig successfull");
...
status_t OMXCameraAdapter::setMeteringAreas(Gen3A_settings& Gen3A)
{
  ...
  if (!meteringAreas)
      {
      LOGE("Error allocating buffer for metering areas %d", eError);
      return -ENOMEM;
      }
  ...
  for ( unsigned int n = 0; n < mMeteringAreas.size(); n++)
      ...
      LOGE("Metering area %d : top = %d left = %d width = %d height = %d prio = %d",
  ...
  if ( NULL == sharedBuffer.pSharedBuff )
      {
      LOGE("No resources to allocate OMX shared buffer");
      ret = -ENOMEM;
      goto EXIT;
      }
  ...
  if ( OMX_ErrorNone != eError )
      {
      LOGE("Error while setting Focus Areas configuration 0x%x", eError);
      ret = -EINVAL;
      }
  else
      {
      LOGE("Metering Areas SetConfig successfull.");
      }
        ...
        if( currSett & mPending3Asettings )
            ...
            switch( currSett )
                ...
                case SetMeteringAreas:
                  ...
                  break;
                default:
                    LOGE("this setting (0x%x) is still not supported in CameraAdapter ",
                                 currSett);
                    break;
                }
