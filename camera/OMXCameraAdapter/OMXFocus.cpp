                                              ...
                                              BaseCameraAdapter::AdapterState state)
{
    ...
    if ( (NO_ERROR == ret) && CameraArea::areAreasDifferent(mFocusAreas, tempAreas) ) {
        ...
        if ( MAX_FOCUS_AREAS < mFocusAreas.size() ) {
            LOGE("Focus areas supported %d, focus areas set %d",
                         MAX_FOCUS_AREAS,
                         mFocusAreas.size());
            ret = -EINVAL;
        }
...
status_t OMXCameraAdapter::doAutoFocus()
{
    ...
    if ( OMX_StateInvalid == mComponentState )
      {
        LOGE("OMX component in Invalid state");
    ...
    if ( 0 != mDoAFSem.Count() )
        {
        LOGE("Error mDoAFSem semaphore count %d", mDoAFSem.Count());
        return NO_INIT;
        }

    if( ((AF_ACTIVE & getState()) != AF_ACTIVE) && ((AF_ACTIVE & getNextState()) != AF_ACTIVE) ) {
       LOGE("Auto focus got canceled before doAutoFocus could be called");
       return NO_ERROR;
    }
    ...
    if (mParameters3A.FocusLock) {
        ...
        if (set3ALock(mUserSetExpLock, mUserSetWbLock, OMX_FALSE) != NO_ERROR) {
            LOGE("Error Unlocking 3A locks");
        } else {
            LOGE("AE/AWB unlocked successfully");
        }
        ...
        if ( NO_ERROR != ret ) {
            LOGE("Focus status check failed 0x%x!", ret);
            return ret;
        } else {
            LOGE("Focus status check 0x%x!", focusStatus.eFocusStatus);
        }
    }
    ...
    if ( ( focusControl.eFocusControl != OMX_IMAGE_FocusControlAuto ) &&
        ...
        if ( OMX_ErrorNone != eError ) {
            LOGE("Error while starting focus 0x%x", eError);
            return INVALID_OPERATION;
        } else {
            LOGE("Autofocus started successfully");
        }

       if(mDoAFSem.WaitTimeout(AF_CALLBACK_TIMEOUT) != NO_ERROR) {
            //If somethiing bad happened while we wait
            if (mComponentState == OMX_StateInvalid) {
                LOGE("Invalid State after Auto Focus Exitting!!!");
                return EINVAL;
            }

            //Disable auto focus callback from Ducati
            setFocusCallback(false);
            LOGE("Autofocus callback timeout expired");
            ...
            LOGE("Autofocus callback received");
            //Disable auto focus callback from Ducati
            setFocusCallback(false);
...
status_t OMXCameraAdapter::stopAutoFocus()
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while stopping focus 0x%x", eError);
...
{;
    ...
    if ( OMX_ErrorNone != eError ) {
        LOGE("Error while retrieving focus mode 0x%x", eError);
    }
...
status_t OMXCameraAdapter::setFocusCallback(bool enabled)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error registering focus callback 0x%x", eError);
            ret = -1;
            }
        else
            {
            LOGE("Autofocus callback for index 0x%x registered successfully",
                         OMX_IndexConfigCommonFocusStatus);
            }
        }
...
status_t OMXCameraAdapter::returnFocusStatus(bool timeoutReached)
{
    ...
    if( ((AF_ACTIVE & state ) != AF_ACTIVE) && ((AF_ACTIVE & nextState ) != AF_ACTIVE) )
       ...
       LOGE("Not sending focus callback because focus was not started");
       return NO_ERROR;
       }
    ...
    if ( NO_ERROR == ret )
        ...
        if ( !timeoutReached )
            ...
            if ( NO_ERROR != ret )
                {
                LOGE("Focus status check failed!");
                }
            }
        }
...
status_t OMXCameraAdapter::checkFocus(OMX_PARAM_FOCUSSTATUSTYPE *eFocusStatus)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while retrieving focus status: 0x%x", eError);
            ret = -1;
            }
        }
    ...
    if ( NO_ERROR == ret )
        {
        LOGE("Focus Status: %d", eFocusStatus->eFocusStatus);
        }
...
status_t OMXCameraAdapter::updateFocusDistances(CameraParameters &params)
{
    ...
    if ( NO_ERROR == ret)
        ...
        ret = addFocusDistances(focusNear, focusOptimal, focusFar, params);
            if ( NO_ERROR != ret )
                {
                LOGE("Error in call to addFocusDistances() 0x%x", ret);
                }
        }
    else
        {
        LOGE("Error in call to getFocusDistances() 0x%x", ret);
        }
...
status_t OMXCameraAdapter::getFocusDistances(OMX_U32 &near,OMX_U32 &optimal, OMX_U32 &far)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while querying focus distances 0x%x", eError);
            ret = UNKNOWN_ERROR;
            }
                                             ...
                                             CameraParameters& params)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( NO_ERROR != ret )
            {
            LOGE("Error encoding near focus distance 0x%x", ret);
            }
        }
...
status_t OMXCameraAdapter::setTouchFocus()
{
    ...
    if ( NO_ERROR == ret )
        ...
        if (!focusAreas)
            {
            LOGE("Error allocating buffer for focus areas %d", eError);
            return -ENOMEM;
            }
        ...
        for ( unsigned int n = 0; n < mFocusAreas.size(); n++)
            ...
            focusAreas[0]->tAlgoAreas[n].nPriority = mFocusAreas.itemAt(n)->getWeight();
             ...
             LOGE("Focus area %d : top = %d left = %d width = %d height = %d prio = %d",
