...
status_t OMXCameraAdapter::doZoom(int index)
{
    ...
    if (  ( 0 > index) || ( ( ZOOM_STAGES - 1 ) < index ) )
        {
        LOGE("Zoom index %d out of range", index);
        ret = -EINVAL;
        }
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while applying digital zoom 0x%x", eError);
            ret = -1;
            }
        else
            {
            LOGE("Digital zoom applied successfully");
...
status_t OMXCameraAdapter::advanceZoom()
{
    ...
    if ( mReturnZoomStatus )
        ...
        if ( ZOOM_ACTIVE & state )
            ...
            if ( mCurrentZoomIdx == mTargetZoomIdx )
                {
                LOGE("[Goal Reached] Smooth Zoom notify currentIdx = %d, targetIdx = %d",
                             mCurrentZoomIdx,
                             mTargetZoomIdx);
                ...
                {
                LOGE("[Advancing] Smooth Zoom notify currentIdx = %d, targetIdx = %d",
                             mCurrentZoomIdx,
                             mTargetZoomIdx);
...
status_t OMXCameraAdapter::startSmoothZoom(int targetIdx)
{
    ...
    LOGE("Start smooth zoom target = %d, mCurrentIdx = %d",
                 targetIdx,
                 mCurrentZoomIdx);
    ...
    if ( ( targetIdx >= 0 ) && ( targetIdx < ZOOM_STAGES ) )
        ...
        {
        LOGE("Smooth value out of range %d!", targetIdx);
        ret = -EINVAL;
        }
...
status_t OMXCameraAdapter::stopSmoothZoom()
{
    ...
    if ( mTargetZoomIdx != mCurrentZoomIdx )
        ...
        mReturnZoomStatus = true;
        LOGE("Stop smooth zoom mCurrentZoomIdx = %d, mTargetZoomIdx = %d",
                     mCurrentZoomIdx,
                     mTargetZoomIdx);
        }
