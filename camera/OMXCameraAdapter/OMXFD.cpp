...
status_t OMXCameraAdapter::setFaceDetection(bool enable, OMX_U32 orientation)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while configuring face detection 0x%x", eError);
            ret = -1;
            }
        else
            {
            LOGE("Face detection configured successfully");
            }
        }
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while configuring face detection extra data 0x%x",
                         eError);
            ret = -1;
            }
        else
            {
            LOGE("Face detection extra data configured successfully");
            }
        }
                                       ...
                                       size_t previewHeight)
{
    ...
    if ( NULL != platformPrivate ) {
        if ( sizeof(OMX_TI_PLATFORMPRIVATE) == platformPrivate->nSize ) {
            LOGE("Size = %d, sizeof = %d, pAuxBuf = 0x%x, pAuxBufSize= %d, pMetaDataBufer = 0x%x, nMetaDataSize = %d",
            ...
            LOGE("OMX_TI_PLATFORMPRIVATE size mismatch: expected = %d, received = %d",
                         ( unsigned int ) sizeof(OMX_TI_PLATFORMPRIVATE),
                         ( unsigned int ) platformPrivate->nSize);
            ret = -EINVAL;
        }
    }  else {
        LOGE("Invalid OMX_TI_PLATFORMPRIVATE");
        return-EINVAL;
    }


    if ( 0 >= platformPrivate->nMetaDataSize ) {
        LOGE("OMX_TI_PLATFORMPRIVATE nMetaDataSize is size is %d",
    ...
    if ( NULL != extraData ) {
        LOGE("Size = %d, sizeof = %d, eType = 0x%x, nDataSize= %d, nPortIndex = 0x%x, nVersion = 0x%x",
                     extraData->nSize,
        ...
        LOGE("Invalid OMX_OTHER_EXTRADATATYPE");
        return -EINVAL;
    }
    ...
    if ( NULL != faceData ) {
        if ( sizeof(OMX_FACEDETECTIONTYPE) == faceData->nSize ) {
            ...
            LOGE("OMX_FACEDETECTIONTYPE size mismatch: expected = %d, received = %d",
                         ( unsigned int ) sizeof(OMX_FACEDETECTIONTYPE),
                         ( unsigned int ) faceData->nSize);
            return -EINVAL;
        }
    } else {
        LOGE("Invalid OMX_FACEDETECTIONTYPE");
        return -EINVAL;
    }
                                                 ...
                                                 size_t previewHeight)
{
    ...
    if ( NULL == faceData ) {
        LOGE("Invalid OMX_FACEDETECTIONTYPE parameter");
        return EINVAL;
    }
