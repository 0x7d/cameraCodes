                                             ...
                                             BaseCameraAdapter::AdapterState state)
{
    ...
    CaptureMode capMode;
    LOGE("Capture mode %s",  params.get(TICameraParameters::KEY_CAP_MODE));
    ...
    LOGE("Capture Mode set %d", mCapMode);
    ...
    if ( valstr != NULL )
        ...
        LOGE("AutoConvergenceMode %s, value = %d", valstr, (int) manualconvergence);
        }
                                              ...
                                              OMX_S32 *pManualConverence)
{
    ...
    if ( eError != OMX_ErrorNone )
        {
        LOGE("Error while getting AutoConvergence 0x%x", eError);
        ret = -EINVAL;
        }
    else
        {
        ...
        *pACMode = ACParams.eACMode;
        LOGE("AutoConvergence got successfully");
        }
                                              ...
                                              OMX_S32 pManualConverence)
{
    ...
    if ( eError != OMX_ErrorNone )
        {
        LOGE("Error while setting AutoConvergence 0x%x", eError);
        ret = -EINVAL;
        }
    else
        {
        LOGE("AutoConvergence applied successfully");
        }
...
status_t OMXCameraAdapter::enableVideoNoiseFilter(bool enable)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while configuring video noise filter 0x%x", eError);
            ret = -1;
            }
        else
            {
            LOGE("Video noise filter is configured successfully");
            }
        }
...
status_t OMXCameraAdapter::enableVideoStabilization(bool enable)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while configuring video stabilization param 0x%x", eError);
            ret = -1;
            }
        else
            {
            LOGE("Video stabilization param configured successfully");
            }
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while getting video stabilization mode 0x%x",
                         (unsigned int)eError);
            ret = -1;
            }

        LOGE("VSTAB Port Index = %d", (int)frameStabCfg.nPortIndex);

        frameStabCfg.nPortIndex = mCameraAdapterParameters.mPrevPortIndex;
        if ( enable )
            {
            LOGE("VSTAB is enabled");
            frameStabCfg.bStab = OMX_TRUE;
            }
        else
            {
            LOGE("VSTAB is disabled");
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while configuring video stabilization mode 0x%x", eError);
            ret = -1;
            }
        else
            {
            LOGE("Video stabilization mode configured successfully");
            }
        }
...
status_t OMXCameraAdapter::setGBCE(OMXCameraAdapter::BrightnessMode mode)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while setting GBCE 0x%x", eError);
            }
        else
            {
            LOGE("GBCE configured successfully 0x%x", mode);
            }
        }
...
status_t OMXCameraAdapter::setGLBCE(OMXCameraAdapter::BrightnessMode mode)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while configure GLBCE 0x%x", eError);
            }
        else
            {
            LOGE("GLBCE configured successfully");
            }
        }
...
status_t OMXCameraAdapter::setCaptureMode(OMXCameraAdapter::CaptureMode mode)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( mSensorIndex == OMX_TI_StereoSensor )
            ...
            {
            LOGE("Camera mode: VIDEO MODE");
            camMode.eCamOperatingMode = OMX_CaptureVideo;
            }
        else
            {
            LOGE("Camera mode: INVALID mode passed!");
            return BAD_VALUE;
            }
        ...
        if( NO_ERROR == ret )
            ...
            if ( OMX_ErrorNone != eError )
                {
                LOGE("Error while configuring ZSL History len 0x%x", eError);
                ...
                {
                LOGE("ZSL History len configured successfully");
                }
            }
        ...
        if( NO_ERROR == ret )
            ...
            if ( OMX_ErrorNone != eError )
                {
                LOGE("Error while configuring camera mode 0x%x", eError);
                ret = ErrorUtils::omxToAndroidError(eError);
                }
            else
                {
                LOGE("Camera mode configured successfully");
                }
            }
        ...
        if( NO_ERROR == ret )
            ...
            if ( OMX_ErrorNone != eError )
                {
                LOGE("Error while configuring CAC 0x%x", eError);
                ret = ErrorUtils::omxToAndroidError(eError);
                }
            else
                {
                LOGE("CAC configured successfully");
                }
            }
        }
...
status_t OMXCameraAdapter::setLDC(OMXCameraAdapter::IPPMode mode)
{
    ...
    if ( NO_ERROR == ret )
        ...
        LOGE("Configuring LDC mode 0x%x", bOMX.bEnabled);
        eError = OMX_SetParameter(mCameraAdapterParameters.mHandleComp,
                                  ( OMX_INDEXTYPE ) OMX_IndexParamLensDistortionCorrection,
                                  &bOMX);
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while setting LDC");
            ret = -1;
            }
        }
...
status_t OMXCameraAdapter::setNSF(OMXCameraAdapter::IPPMode mode)
{
    ...
    if ( NO_ERROR == ret )
        ...
        LOGE("Configuring NSF mode 0x%x", nsf.eMode);
        eError = OMX_SetParameter(mCameraAdapterParameters.mHandleComp,
                                  (OMX_INDEXTYPE)OMX_IndexParamHighISONoiseFiler,
                                  &nsf);
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while setting NSF");
            ret = -1;
            }
        }
...
status_t OMXCameraAdapter::setImageQuality(unsigned int quality)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while configuring jpeg Quality 0x%x", eError);
            ret = -1;
            }
        }
                                              ...
                                              unsigned int quality)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while retrieving thumbnail size 0x%x", eError);
            ret = -1;
            }
        ...
        LOGE("Thumbnail width = %d, Thumbnail Height = %d", width, height);
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while configuring thumbnail size 0x%x", eError);
            ret = -1;
            }
        }
                                           ...
                                           bool enable)
{
    ...
    if ( FACE_PRIORITY == priority ) {
        ...
        eError =  OMX_SetConfig(mCameraAdapterParameters.mHandleComp,
                                ...
                                &mFacePriority);
        if ( OMX_ErrorNone != eError ) {
            LOGE("Error while configuring face priority 0x%x", eError);
        } else {
            LOGE("Face priority for algorithms set successfully 0x%x, 0x%x, 0x%x",
        ...
        eError =  OMX_SetConfig(mCameraAdapterParameters.mHandleComp,
                                ...
                                &mRegionPriority);
        if ( OMX_ErrorNone != eError ) {
            LOGE("Error while configuring region priority 0x%x", eError);
        } else {
            LOGE("Region priority for algorithms set successfully 0x%x, 0x%x, 0x%x",
...
status_t OMXCameraAdapter::setPictureRotation(unsigned int degree)
{
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while configuring rotation 0x%x", eError);
            }
        }
...
status_t OMXCameraAdapter::setSensorOrientation(unsigned int degree)
{
    ...
    */
    ...
    if ( NO_ERROR == ret )
        ...
        if ( OMX_ErrorNone != eError )
            {
            LOGE("Error while Reading Sensor Orientation :  0x%x", eError);
            }
        LOGE(" Currently Sensor Orientation is set to : %d",
        ...
        LOGE(" Read the Parameters that are set");
        eError = OMX_GetConfig(mCameraAdapterParameters.mHandleComp,
                               OMX_IndexConfigCommonRotate,
                               &sensorOrientation);
        if ( OMX_ErrorNone != eError )
        ...
        LOGE(" Currently Sensor Orientation is set to : %d",
                     ( unsigned int ) sensorOrientation.nRotation);
        LOGE(" Sensor Configured for Port : %d",
...
status_t OMXCameraAdapter::setVFramerate(OMX_U32 minFrameRate, OMX_U32 maxFrameRate)
{
    ...
    if ( NO_ERROR == ret ) {
        ...
        eError = OMX_SetConfig(mCameraAdapterParameters.mHandleComp,
                               ...
                               &vfr);
        if(OMX_ErrorNone != eError) {
            LOGE("Error while setting VFR min = %d, max = %d, error = 0x%x",
                         ( unsigned int ) minFrameRate,
                         ( unsigned int ) maxFrameRate,
                         eError);
            ret = -1;
        } else {
            LOGE("VFR Configured Successfully [%d:%d]",
                        ( unsigned int ) minFrameRate,
                        ( unsigned int ) maxFrameRate);
        }
    }
