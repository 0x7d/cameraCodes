...
#define AVIF_COPYRIGHTED        0x00020000
    ...
    if (lseek(AVI->fdes, 0, SEEK_SET) < 0
            || avi_write(AVI->fdes, (char *) AVI_header, HEADERBYTES)
            != HEADERBYTES
...
static int avi_close_output_file(avi_t *AVI) {
    ...
    ret = avi_add_chunk(AVI, (unsigned char *) "idx1",
            (unsigned char *) AVI->idx, AVI->n_idx * 16);
    hasIndex = (ret == 0);
...
#define AVIF_COPYRIGHTED        0x00020000
    ...
    if (lseek(AVI->fdes, 0, SEEK_SET) < 0
            || avi_write(AVI->fdes, (char *) AVI_header, HEADERBYTES)
            != HEADERBYTES || ftruncate(AVI->fdes, AVI->pos) < 0) {
        AVI_errno = AVI_ERR_CLOSE;
        return -1;
    }
...
int avi_parse_input_file(avi_t *AVI, int getIndex) {
                    ...
                    for (i = 0; i < hdrl_len;) {
                        ...
                        if (strncasecmp((char *) hdrl_data + i, "strh", 4) == 0) {
                            ...
                            if (strncasecmp((char *) hdrl_data + i, "vids", 4) == 0
                                ...
                                AVI->track[AVI->aptr].audio_bytes = str2ulong(
                                        hdrl_data + i + 32) * avi_sampsize(AVI, 0);
                            ...
                            if (lasttag == 1) {
                                ...
                                AVI->bitmap_info_header = (BITMAPINFOHEADER_avilib *) malloc(
                                        bih.bi_size);
                                ...
                                wfe = (WAVEFORMATEX_avilib *) malloc(
                                        sizeof(WAVEFORMATEX_avilib));
                                if (wfe != NULL) {
                                    memset(wfe, 0, sizeof(WAVEFORMATEX_avilib));
                                    memcpy(wfe, hdrl_data + i, wfes);
                                    if (wfe->cb_size != 0) {
                                        nwfe = (char *) realloc(wfe,
                                                sizeof(WAVEFORMATEX_avilib) + wfe->cb_size);
                                        if (nwfe != 0) {
                                ...
                                AVI->track[AVI->aptr].mp3rate = 8 * str2ulong(hdrl_data + i + 8)
                                    / 1000;
                                //:ThOe
    ...
    for (j = 0; j < AVI->anum; ++j) {
        if (AVI->track[j].audio_chunks) {
            AVI->track[j].audio_index = (audio_index_entry *) malloc(
    ...
    for (i = 0; i < AVI->n_idx; i++) {
        ...
        for (j = 0; j < AVI->anum; ++j) {
            ...
            if (strncasecmp((char *) AVI->idx[i], AVI->track[j].audio_tag, 4)
                    == 0) {
                AVI->track[j].audio_index[nai[j]].pos = str2ulong(
                        AVI->idx[i] + 8) + ioff;
                AVI->track[j].audio_index[nai[j]].len = str2ulong(
                        AVI->idx[i] + 12);
...
int AVI_set_audio_position(avi_t *AVI, long byte) {
    ...
    AVI->track[AVI->aptr].audio_posb = byte
        - AVI->track[AVI->aptr].audio_index[n0].tot;
...
long AVI_read_audio(avi_t *AVI, char *audbuf, long bytes) {
    ...
    while (bytes > 0) {
        left =
            AVI->track[AVI->aptr].audio_index[AVI->track[AVI->aptr].audio_posc].len
        ...
        if (bytes < left)
            ...
            todo = left;
        pos =
            AVI->track[AVI->aptr].audio_index[AVI->track[AVI->aptr].audio_posc].pos
            + AVI->track[AVI->aptr].audio_posb;
        lseek(AVI->fdes, pos, SEEK_SET);
...
long AVI_read_audio_chunk(avi_t *AVI, char *audbuf) {
    ...
    if (AVI->track[AVI->aptr].audio_index[AVI->track[AVI->aptr].audio_posc].len
            == 0)
        return 0;
    left =
        AVI->track[AVI->aptr].audio_index[AVI->track[AVI->aptr].audio_posc].len
