    ...
    for (queryctrl.id = V4L2_CID_BASE; queryctrl.id < V4L2_CID_LASTP1;
        ...
        if (0 == ioctl(vd, VIDIOC_QUERYCTRL, &queryctrl)) {
            ...
            printf(
                    ...
                    queryctrl.id, queryctrl.name, queryctrl.type,
        ...
        for (queryctrl.id = V4L2_CID_BASE; queryctrl.id < V4L2_CID_LASTP1;
            ...
            if (0 == ioctl(vd, VIDIOC_QUERYCTRL, &queryctrl)) {
                ...
                fprintf(
                        ...
                        queryctrl.id, control_s.value, queryctrl.name,
                        queryctrl.type, queryctrl.minimum, queryctrl.maximum,
...
static int init_v4l2(struct vdIn *vd) {
    ...
    for (i = 0; i < NB_BUFFER; i++) {
        ...
        vd->mem[i] = mmap(0 /* start anywhere */, vd->buf.length, PROT_READ,
...
end_capture: if (frame)
    ...
    if (vd->captureFile && vd->buf.bytesused > 0) {
        int ret;
        ret = fwrite(vd->mem[vd->buf.index], vd->buf.bytesused, 1,
                vd->captureFile);
        if (ret < 1) {
        ...
        if (vd->toggleAvi) {
            ...
            if (vd->avifile == NULL) {
                ...
                if (vd->avifile == NULL) {
                    ...
                    AVI_set_video(vd->avifile, vd->width, vd->height, 15,
                            "MJPG");
                    printf("recording to %s\n", vd->avifilename);
                }
            } else {
                /* if we have a valid avifile, record the frame to it */
                AVI_write_frame(vd->avifile, vd->tmpbuffer, vd->buf.bytesused,
                        vd->framecount);
                vd->framecount++;
            }
        }
